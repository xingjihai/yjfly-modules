...
//                    int port = (infos.length > 1)?Integer.parseInt(infos[1]):6379;
    ...
    public BinaryJedisCommands get() {
        try {
            if(!isHostConnectable()){
                return new NullJedisCommands();   //redis 宕机后使系统正常运行 -- 让系统查询数据库
            }
    ...
    private boolean isHostConnectable=true;

    public  boolean isHostConnectable() {
        Socket socket = new Socket();

        long newCheckTime=Calendar.getInstance().getTimeInMillis();
        if( newCheckTime-checkTime<10000  ){  //每十秒再重新检测
...
//            e.printStackTrace();
            log.error("redis("+host+":"+port+") 无法连接 ... ");
