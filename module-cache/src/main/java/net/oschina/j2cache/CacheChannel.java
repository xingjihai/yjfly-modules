...
public abstract class CacheChannel implements Closeable , AutoCloseable {
	...
	public Collection<String> keys(String region)  {
        ...
        Collection<String>  keys1=holder.getLevel1Cache(region).keys();

        long time2=Calendar.getInstance().getTimeInMillis();
        log.debug( "【J2Cache性能测试】 一级缓存获取keys("+keys1.size()+")用时:"+(time2-time1)+"ms" );

		keys.addAll( keys1 );



        Collection<String>  keys2=holder.getLevel2Cache(region).keys();
        long time3=Calendar.getInstance().getTimeInMillis();
        log.debug( "【J2Cache性能测试】二级缓存获取keys("+keys2.size()+")用时:"+(time3-time2)+"ms" );
